...
export function SignalDetailDialog({
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      <DialogContent className="max-h-[92vh] gap-5 overflow-y-auto rounded-2xl border-bq-border bg-bq-card p-6 font-satoshi text-bq-heading sm:max-w-[680px]">
        <DialogHeader className="flex-row items-start gap-3 space-y-0">
          ...
          <div className="flex-1">
            <DialogTitle className="flex items-center gap-2 text-[16px] font-bold text-bq-heading">
              {d.name}
          ...
          <button
            ...
            className="mr-6 flex shrink-0 items-center gap-1.5 rounded-lg border border-bq-border px-3 py-1.5 text-[12px] text-bq-heading transition-colors hover:bg-bq-surface"
          >
          ...
          {d.stats.map((s, i) => {
            ...
            return (
              <div key={s.label} className="rounded-xl border border-bq-border bg-bq-surface/40 p-3">
                ...
                <p className={cn("mt-2 text-[22px] font-bold", s.accent ? "text-bq-mint" : "text-bq-heading")}>
                  {s.value}
                </p>
              </div>
            );
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          <div className="rounded-xl border border-bq-border bg-bq-surface/40 p-4">
            <div className="flex items-center justify-between">
              <p className="text-[13px] font-semibold text-bq-heading">Win Rate Trend</p>
              <span className="text-[12px] font-medium text-bq-mint">85% current</span>
            </div>
            <div className="mt-3">
              <AreaLineChart data={d.trend} color="var(--bq-mint)" height={90} showLabels={false} />
            </div>
          ...
          <div className="rounded-xl border border-bq-border bg-bq-surface/40 p-4">
            <p className="text-[13px] font-semibold text-bq-heading">By Pair</p>
            <div className="mt-3 space-y-2.5">
              {d.byPair.map((p) => (
                <div key={p.pair} className="flex items-center gap-2.5 text-[12px]">
                  ...
                  <span className="w-8 shrink-0 text-right font-medium text-bq-heading tabular-nums">{p.pct}%</span>
                </div>
              ))}
            </div>
          </div>
        ...
        <div>
          <p className="mb-2 text-[13px] font-semibold text-bq-heading">Recent Signals</p>
              ...
              <tbody>
                {d.recent.map((s, i) => (
                  <tr key={i} className="border-t border-bq-border-soft">
                    ...
                    <td className="py-2.5 font-medium text-bq-heading">{s.pair}</td>
                    <td className={cn("py-2.5 font-semibold", s.dir === "BUY" ? "text-bq-mint" : "text-bq-loss-text")}>
                      {s.dir}
                    </td>
                    <td className="py-2.5 text-bq-heading tabular-nums">{s.entry}</td>
                    <td className="py-2.5 text-bq-mint tabular-nums">{s.tp}</td>
                    <td className="py-2.5 text-bq-loss-text tabular-nums">{s.sl}</td>
                    <td className="py-2.5 text-right">
                      <span
                        className={cn(
                          ...
                          s.status === "SL Hit" && "bg-bq-loss/10 text-bq-loss-text",
                        )}
                      >
                        {s.status}
                      </span>
        ...
        <div className="flex flex-wrap gap-3">
          <DialogClose asChild>
            <Button
              variant="outline"
              className="h-auto flex-1 rounded-xl border-bq-border bg-transparent py-2.5 text-[13px] font-semibold text-bq-heading hover:bg-bq-surface dark:border-bq-border dark:bg-transparent dark:hover:bg-bq-surface"
            >
              Close
            </Button>
          </DialogClose>
          <Button
            ...
            className="h-auto flex-1 gap-2 rounded-xl border-bq-border bg-transparent py-2.5 text-[13px] font-semibold text-bq-heading hover:bg-bq-surface dark:border-bq-border dark:bg-transparent dark:hover:bg-bq-surface"
          >
            <Settings className="size-4" /> Edit Settings
          </Button>
          <Button
            onClick={() => toast("Open Full View", { description: "Strategy full view coming soon." })}
            className="h-auto flex-[1.2] gap-2 rounded-xl bg-bq-contrast py-2.5 text-[13px] font-semibold text-bq-on-fill hover:bg-bq-contrast/90"
          >
